...
export default function ArticleEditor({ articleId, onSave, onCancel }: ArticleEditorProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const { clientAuthorizedFetch } = await import('@/lib/auth-fetch-client');

        // Load categories first
        const categoriesResponse = await clientAuthorizedFetch('/api/admin/categories', {
          headers: {
        ...
        if (articleId) {
          const articleResponse = await clientAuthorizedFetch(`/api/admin/articles/${articleId}`, {
            headers: {
              'Content-Type': 'application/json'
            }
          });
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const { clientAuthorizedFetch } = await import('@/lib/auth-fetch');
