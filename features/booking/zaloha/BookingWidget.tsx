...
export function BookingWidget() {
  ...
  interface RawBlockedTermItem extends Partial<BlockedTermItem> {
    datum_od?: string;
    datum_do?: string;
  }
  ...
  const loadReservations = async () => {
    console.log('🟢 loadReservations - spuštěno');
    setLoading(true);
    try {
      // Načteme všechny budoucí rezervace (bez časového omezení)
      const today = new Date();
      const startDate = formatLocalDate(today); // dnešní datum
      
      const queryString = `/api/rezervace?datum_od=${startDate}`;
      console.log('📥 Volám API:', queryString);
      
      const response = await fetch(queryString);
      
      if (response.ok) {
        const data = await response.json();
        console.log(`✅ Načteno ${data.rezervace?.length || 0} rezervací z API:`, data.rezervace);
  ...
  const filteredRezervace = useMemo(() => {
    ...
    return rezervace.filter((r) => {
      ...
      if (!stavMatch || !dateFromMatch || !dateToMatch) {
        return false;
      }
  ...
  const resetFilters = () => {
    setFilters({ search: '', stav: 'all', datumOd: '', datumDo: '' });
  };
  ...
  const handleSelectReservation = (id: number, checked: boolean) => {
    console.log(`Selecting reservation ${id}: ${checked}`);
    if (checked) {
      setSelectedReservations(prev => {
        const newSelection = [...prev, id];
        console.log('New selected reservations:', newSelection);
        return newSelection;
      });
    } else {
      setSelectedReservations(prev => {
        const newSelection = prev.filter(resId => resId !== id);
        console.log('New selected reservations:', newSelection);
        return newSelection;
      });
    }
  ...
  const handleDeleteSelected = async () => {
    console.log('handleDeleteSelected voláno, vybrané rezervace:', selectedReservations);
    
    if (selectedReservations.length === 0) {
      console.log('Žádné rezervace nejsou vybrané');
      return;
    }
    ...
    showConfirmDialog(
      ...
      async () => {
        try {
          console.log('Odesílám DELETE request s IDs:', selectedReservations);
  ...
  // Debug informace
  console.log('Debug - selectedReservations:', selectedReservations);
  console.log('Debug - rezervace.length:', rezervace.length);
  console.log('Debug - isSomeSelected:', isSomeSelected);
  console.log('Debug - isAllSelected:', isAllSelected);
  ...
  const [editDraft, setEditDraft] = useState({
    ...
    stav: 'pending', cena: 0, zpusobPlatby: '',
  ...
  const handleEditReservation = async (rezervace: Rezervace) => {
    ...
    const zamestnanecId = String((rezervace as any).zamestnanecId || (rezervace as any).zamestnanec?.id || '');
    const sluzbaId = String((rezervace as any).sluzbaId || (rezervace as any).sluzba?.id || '');
    const casOd = rezervace.casOd || (rezervace as any).cas_od || '';
    const casDo = rezervace.casDo || (rezervace as any).cas_do || '';
  ...
  const handleReservationFormSuccess = async () => {
    console.log('handleReservationFormSuccess - spuštěno');
    // Malá prodleva aby se zajistilo, že database na serveru je aktualizovaná
    setTimeout(async () => {
      console.log('handleReservationFormSuccess - spouštění loadReservations po prodlevě');
      await loadReservations();
    }, 500);
    // Zavřít dialog bez čekání na načtení - formulář se přepne v handleSubmit
  };
  ...
  const getStatusBadge = (stav: string) => {
    const statusConfig = {
      ...
      confirmed: { label: 'Potvrzeno', variant: 'default' as const, className: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' },
      ...
      // Nové stavy
      potvrzeno: { label: 'Potvrzeno', variant: 'default' as const, className: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' },
      dokonceno: { label: 'Dokončeno', variant: 'outline' as const, className: 'bg-blue-200 text-blue-800 dark:bg-blue-900 dark:text-blue-200' },
      zruseno_zakaznikem: { label: 'Zrušeno zákazníkem', variant: 'destructive' as const, className: 'bg-red-200 text-red-800 dark:bg-red-900 dark:text-red-200' },
      zruseno_salonem: { label: 'Zrušeno salonem', variant: 'destructive' as const, className: 'bg-red-200 text-red-800 dark:bg-red-900 dark:text-red-200' },
      nedorazil: { label: 'Nedorazil', variant: 'destructive' as const, className: 'bg-orange-200 text-orange-800 dark:bg-orange-900 dark:text-orange-200' }
    };
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="space-y-2">
                  ...
                  <Badge variant="outline" className="text-amber-700 border-white bg-amber-50">
                    Poznámka: rezervace ve stavu "Čeká na potvrzení" neblokují termíny.
                  </Badge>
                </div>
              ...
              <div className="overflow-x-auto">
                ...
                <TableBody>
                  {filteredRezervace.map((rezervace) => (
                    <TableRow key={rezervace.id} className={selectedReservations.includes(rezervace.id) ? "bg-accent border-accent-foreground" : ""}>
                      <TableCell>
                        <input
                          type="checkbox"
                          checked={selectedReservations.includes(rezervace.id)}
                          onChange={(e) => handleSelectReservation(rezervace.id, e.target.checked)}
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                          aria-label={`Vybrat rezervaci ${rezervace.jmeno} ${rezervace.prijmeni}`}
                        />
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className="font-semibold text-[#A39566] border-[#D8CDAA] bg-[#F8F4EA]">
                          #{rezervace.id}
                        </Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="font-medium">{new Date(rezervace.datum).toLocaleDateString('cs-CZ', { day: 'numeric', month: 'numeric', year: 'numeric' })}</div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="max-w-45">
                        <div className="max-w-full">
                          <div 
                            ...
                            title={rezervace.sluzby || rezervace.sluzba?.nazev || 'Služba není specifikována'}
                          >
                            {rezervace.sluzby || rezervace.sluzba?.nazev || 'Služba není specifikována'}
                          </div>
                          {rezervace.poznamka && (
                            <div 
                      ...
                      <TableCell>
                        <div className="font-medium text-sm">                          {rezervace.kadernice || rezervace.zamestnanec?.jmeno || 'Nepřiřazeno'}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          ...
                          {(rezervace.pocetDeti || 0) > 0 && (
                            <div className="text-xs text-muted-foreground">{rezervace.pocetDeti} dětí</div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
              ...
              <CardContent>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-lg font-medium">Správa blokovaných termínů</h3>
                      <p className="text-sm text-muted-foreground">Blokujete dny, kdy nelze rezervovat (dovolená, údržba).</p>
                    </div>
                    <div>
                        ...
                        <div key={b.id} className="flex items-center justify-between p-3 bg-muted rounded">
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              {new Date(b.datumOd).toLocaleDateString()}
                              {new Date(b.datumOd).toDateString() !== new Date(b.datumDo).toDateString() && (
                                <span> - {new Date(b.datumDo).toLocaleDateString()}</span>
                              )}
                              {new Date(b.datumOd).toDateString() === new Date(b.datumDo).toDateString() && (
            ...
            <div>
              ...
              <Select value={filters.stav} onValueChange={(value) => setFilters((prev) => ({ ...prev, stav: value }))}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="potvrzeno">Potvrzeno (CZ)</SelectItem>
                  <SelectItem value="dokonceno">Dokončeno (CZ)</SelectItem>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {detailRezervace && (
          <div className="space-y-4 text-sm">
            {/* Klient */}
            <div>
              <p className="text-xs font-semibold uppercase text-muted-foreground mb-2 tracking-wide">Klient</p>
              <div className="grid grid-cols-2 gap-1">
                <span className="text-muted-foreground">Jméno</span>
                <span className="font-medium">{detailRezervace.jmeno} {detailRezervace.prijmeni}</span>
                <span className="text-muted-foreground">E-mail</span>
                <span className="font-medium">{detailRezervace.email}</span>
                <span className="text-muted-foreground">Telefon</span>
                <span className="font-medium">{detailRezervace.telefon}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1">
                ...
                <span className="font-medium">{formatDate(detailRezervace.datum)}</span>
                <span className="text-muted-foreground">Čas</span>
                <span className="font-medium">
                  {(detailRezervace.casOd || (detailRezervace as any).cas_od || '-')}
                  {' – '}
                  {(detailRezervace.casDo || (detailRezervace as any).cas_do || '-')}
                </span>
              </div>
            </div>
            ...
            {/* Sluzba a kadernice */}
            <div>
              <p className="text-xs font-semibold uppercase text-muted-foreground mb-2 tracking-wide">Služba a kadeřnice</p>
              <div className="grid grid-cols-2 gap-1">
                <span className="text-muted-foreground">Služb{(() => {
                ...
                <span className="text-muted-foreground">Kadeřnice</span>
                <span className="font-medium">{detailRezervace.kadernice || detailRezervace.zamestnanec?.jmeno || 'Nepřiřazeno'}</span>
              </div>
            </div>

            {/* Platba a stav */}
            <div>
              <p className="text-xs font-semibold uppercase text-muted-foreground mb-2 tracking-wide">Platba a stav</p>
              <div className="grid grid-cols-2 gap-1">
                <span className="text-muted-foreground">Stav</span>
                <span>{getStatusBadge(detailRezervace.stav)}</span>
                <span className="text-muted-foreground">Cena</span>
                <span className="font-medium">{formatCurrency(detailRezervace.cena)}</span>
                {detailRezervace.zpusobPlatby && (
                  <>
                    <span className="text-muted-foreground">Způsob platby</span>
                    <span className="font-medium capitalize">{detailRezervace.zpusobPlatby}</span>
                  </>
                )}
              </div>
            </div>
            ...
            {((detailRezervace.pocetOsob && detailRezervace.pocetOsob > 1) || detailRezervace.pocetDeti || detailRezervace.poznamka) && (
              <div>
                <p className="text-xs font-semibold uppercase text-muted-foreground mb-2 tracking-wide">Doplňkové informace</p>
                <div className="grid grid-cols-2 gap-1">
                  {detailRezervace.pocetOsob && detailRezervace.pocetOsob > 0 && (
                    <>
                      <span className="text-muted-foreground">Počet osob</span>
                      <span className="font-medium">{detailRezervace.pocetOsob}</span>
                    </>
                  )}
                  {(detailRezervace.pocetDeti ?? 0) > 0 && (
                    <>
                      <span className="text-muted-foreground">Počet dětí</span>
                      <span className="font-medium">{detailRezervace.pocetDeti}</span>
                    </>
                  )}
